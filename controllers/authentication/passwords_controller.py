...
from models.authentication_model import Password
...
class PasswordsController:
    ...
    async def get_active_password(self, user_id: int) -> str:
        ...
        """
        query = select(Password).where(Password.user_id == user_id, Password.is_active)
        password = await self.session.execute(query)
    ...
    async def check_password(self, user_id: int, password_to_check: str) -> bool:
        ...
        hashed_password_to_check = Password.set_password(password_to_check)

        return current_active_password == hashed_password_to_check

    async def create_password(self, user_id: int, password: str, expiration_date: date = None) -> Password:
        ...
        if await self.is_there_active_password_for_user(user_id):
        ...
        db_password = Password(user_id=user_id,
    ...
    async def is_there_active_password_for_user(self, user_id: int) -> bool:
    ...
    async def update_password(self, user_id: int, current_password: str, new_password: str,
                              expiration_date: date = None) -> Password:
        ...
        if not await self.is_there_active_password_for_user(user_id):
            raise HTTPException(status_code=400,
